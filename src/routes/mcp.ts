...
let activeObservability: McpObservability | undefined;
...
/**
 * Seed the observability instance (called once at startup).
 */
export function seedObservability(obs: McpObservability) {
...
export async function mcpRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const { mcpServer } = createMCPServer(activeConfig, brandKnowledge, sessionId, activeObservability);

      sessions.set(sessionId, { mcpServer, transport });
      app.log.info({ sessionId }, "[mcp] SSE connection established");
      activeObservability?.recordSession({ type: "mcp_session", action: "open", sessionId });
      ...
      transport.onclose = async () => {
        ...
        activeObservability?.recordSession({ type: "mcp_session", action: "close", sessionId });
