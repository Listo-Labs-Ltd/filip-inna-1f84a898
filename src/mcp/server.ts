...
import type { McpObservability } from "@listo-ai/mcp-observability";
...
export function createMCPServer(
  ...
  // ── Observability helper — wraps a tool handler when observability is present ──
  function tracked(toolName: string, handler: (args: any) => Promise<any>) {
    if (!observability) return handler;
    return observability.wrapMcpHandler("CallTool", handler, (args: any) => ({
      toolName,
      sessionId,
  ...
  mcpServer.registerTool(
    ...
    tracked("search_products", async (args: any) => {
      const t0 = Date.now();
  ...
  mcpServer.registerTool(
    ...
    tracked("get_product", async (args: any) => {
      const t0 = Date.now();
      const stableId = sessionId ? getConversationId(sessionId) : "default";
      args.conversationId = stableId;
  ...
  mcpServer.registerTool(
    ...
    tracked("manage_cart", async (args: any) => {
      const t0 = Date.now();
  ...
  mcpServer.registerTool(
    ...
    tracked("get_checkout_url", async (args: any) => {
      const t0 = Date.now();
      const stableId = sessionId ? getConversationId(sessionId) : "default";
      const usedMapping = stableId !== sessionId;
  ...
  mcpServer.registerTool(
    ...
    tracked("get_support_info", async (args: any) => {
      const t0 = Date.now();
      const stableId = sessionId ? getConversationId(sessionId) : "default";
      args.conversationId = stableId;
  ...
  mcpServer.registerTool(
    ...
    tracked("get_brand_info", async (args: any) => {
      const t0 = Date.now();
      const stableId = sessionId ? getConversationId(sessionId) : "default";
      args.conversationId = stableId;
